...
def save_evaluation_plots(t_te, y_te, out_dir, out_prefix, train_mse, test_mse, test_reg, z=None, title=None):
    ...
    plt.clf()
    plt.plot(t_mean, z, "b-", linewidth=2, label="Bremen") 
    plt.plot(y_mean, z, "r-", linewidth=2, label="ONNI")
    plt.plot(t_mean + t_std, z, "b--", linewidth=1, label="Bremen ±1σ")
    plt.plot(t_mean - t_std, z, "b--", linewidth=1)
    plt.plot(y_mean + y_std, z, "r--", linewidth=1, label="ONNI ±1σ")
    ...
    plt.legend(loc="best")
    if title is None:
        plt.title(f"ONNI(red) vs Bremen(blue); n_test={n_te} train_mse={train_mse:.4g} test_mse={test_mse:.4g} test_reg={test_reg:.6f}")
    else:
        plt.title(str(title))
